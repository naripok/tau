...
class _BlockingReader:
    """Stdin stand-in that blocks on empty reads until marked closed."""
...
@pytest.mark.anyio
async def test_rpc_streams_turn_retry_events_with_pi_aliases(tmp_path: Path) -> None:
    """Prove fork turn-level retries surface on the RPC stream with Pi-style aliases.

    The fork retries transient provider failures inside the agent loop; a
    Pi-compatible frontend must observe the retry and the voided partial so it
    can render turn-level recovery instead of a spurious settled failure.
    """
    provider = FakeProvider(
        [
            [
                assistant_start(model="fake"),
                text_delta("partial"),
                transport_error("peer closed connection", partial="partial"),
            ],
            [
                assistant_start(model="fake"),
                text_delta("recovered"),
                assistant_done(AssistantMessage(content="recovered", model="fake")),
            ],
        ]
    )
    session = await _session(tmp_path, provider)
    stdin = _BlockingReader()
    stdout = StringIO()
    server = RpcServer(session, stdin=stdin, stdout=stdout)  # type: ignore[arg-type]

    def close_after_stream_finishes() -> None:
        # Two phases: first the dispatch must register the prompt task, then
        # the stream must fully settle (including any retry backoff) before
        # EOF is signalled; an early EOF makes the server cancel the run.
        while not server._active_prompt_tasks:
            time.sleep(0.005)
        while server._active_prompt_tasks:
            time.sleep(0.005)
        stdin.close()

    closer = threading.Thread(target=close_after_stream_finishes, daemon=True)
    closer.start()
    stdin.push('{"id":"retry","type":"prompt","message":"hi"}\n')
    await server.run()
    closer.join()

    records = [json.loads(line) for line in stdout.getvalue().splitlines()]
    retry = [record for record in records if record["type"] == "turn_retry_start"]
    assert len(retry) == 1
    assert retry[0]["attempt"] == 2
    assert retry[0]["maxAttempts"] == 3
    assert retry[0]["delaySeconds"] == 0.25
    assert retry[0]["errorMessage"] == "peer closed connection"
    assert len(provider.calls) == 2
    assert records[-1]["type"] == "agent_settled"
    settled = json.dumps(records)
    assert '"recovered"' in settled
