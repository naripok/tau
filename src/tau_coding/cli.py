...
def main(
    ...
    mode: Annotated[
        ...
        typer.Option(
            "--mode",
            help="Run headlessly with this output format (text, json, or transcript).",
        ),
    ] = None,
    output: Annotated[
        PrintOutputMode | None,
    ...
    if not print_requested and not export and command == "update" and len(positional_args) == 1:
        raise typer.BadParameter("Unknown command: update")

    if not print_requested and not export and command == "install":
